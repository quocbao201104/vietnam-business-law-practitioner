...
Determining the tax-relevant characterization of an already resolved business fact pattern, including the material taxable event(s), taxpayer/payer/recipient/withholder roles, and candidate tax regimes that must be resolved under current authority.

This unit owns **tax characterization and statutory tax-role propositions**. It does not reconstruct the underlying contract, employment relationship, ownership state, foreign-payment classification, or customs proposition.
...
- employee/contractor classification — BL6;
...
- what tax-relevant transaction/event actually occurred;
- who may be taxpayer, payer, recipient, withholding party, employer, transferor, transferee, distributor, service recipient, or another tax-relevant actor;
...
- evidence supporting the factual event;
...
If BL6 has not resolved whether a worker is an employee/contractor, BL5 must not choose the classification merely to compute tax.
...
1. **State the tax proposition.** Example: `Who is the statutory withholding party for payment P?` or `What tax characterization applies to transaction T?`
2. **Consume the business fact pattern.** Use committed BL2/BL3/BL6/BL8 propositions. Do not reconstruct them.
3. **Identify the tax event(s).** Separate sale/service/payment/employment/transfer/distribution/import-related or other legally material events.
4. **Identify candidate statutory roles.** Taxpayer, payer, recipient, withholder, employer, transferor/transferee, or another role as supported by current authority.
5. **Identify candidate tax regimes.** Do not decide by transaction nickname alone.
...
8. **Hand downstream within BL5.** Base/rate/timing questions go to `base-method-rate-timing.md`; invoice/document questions go to `documentation-invoice-evidence.md`; structuring/incentive questions go to `incentives-structuring-economics.md` only when material.
...
- statutory registrations/identifiers where role depends on them;
...
- current exemptions/exclusions that affect characterization.

Do not hardcode current statutory definitions, rates, thresholds, exemptions, forms, or article numbers as stable knowledge.
...
Consume transaction terms, payment facts, commercial tax allocation, and performance state. Contractual allocation never substitutes for statutory tax-role analysis.

### From BL6

Consume committed employment classification. If classification is under review, keep dependent tax propositions conditional or `VERIFY_BEFORE_ACTION`; do not promote the competing employment classification yourself.
...
Return committed characterization/event/role propositions. Siblings must not independently reconstruct them unless a contradiction requires review.
...
- BL5 reclassifying worker instead of consuming BL6;
