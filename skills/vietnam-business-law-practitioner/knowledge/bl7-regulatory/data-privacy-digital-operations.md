...
Determining the public-law legality and compliance state of material personal-data, monitoring, tracking, profiling, employee/customer data, digital-platform, device/access, and other data-processing operations after the relevant actor/activity role is sufficiently resolved.

This unit owns **privacy/data/digital-operation propositions**. It does not decide whether an employee committed misconduct, whether a contract was formed, or whether a cross-border payment/trade flow is lawful merely because data or a platform is involved.

## Does not own

- regulatory-perimeter/role discovery when unresolved — `regulatory-perimeter-role.md`;
...
- controller/processor/platform/intermediary or another legally material data role;
...
- committed BL7 perimeter/regulated-role state;
...
2. **Consume actor/perimeter state.** Do not reconstruct employment, contract, corporate, or trade state inside BL7.
...
4. **Identify the legally material data role(s).** Do not assign one global role if roles differ across operations.
...
Use Authority Resolver for current data-role definitions, legal grounds/conditions, transparency/consent/notice requirements, employee/customer monitoring rules, sensitive/special data treatment, data-subject rights, retention/deletion/security requirements, vendor/recipient obligations, cross-border data requirements, breach/incident duties, or historical rules at the processing date.
...
Invoke privacy/security/sector technical specialists only when the proposition materially requires depth. Specialist findings return to BL7 for owner review before shared-state promotion.
