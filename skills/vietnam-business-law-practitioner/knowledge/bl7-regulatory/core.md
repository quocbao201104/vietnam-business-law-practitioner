...
- regulatory perimeter, regulated role, and material regime triggers;
- prohibition, market-entry permission, license/approval/registration/notification, product/service market placement, and ongoing compliance;
- consumer, advertising/claims, e-commerce selling conduct, competition and other market-conduct constraints;
- privacy/data/monitoring/digital-operation legality and compliance;
- BL7-owned specialist activation and integration of candidate specialist findings.
...
- market-entry compliance ≠ ongoing compliance;
...
- which actor/regulated role applies to a proposition;
- whether several regimes overlap;
- whether a material business-model/fact change activates another BL7 route;
- whether specialist depth is justified by a concrete trigger.

### Permission / entry / ongoing compliance

Load `permission-entry-ongoing-compliance.md` when perimeter/role is sufficiently resolved and the question depends on:
...
- ongoing conditions, renewals, reporting, recordkeeping, inspection or event-triggered duties;
...
- data roles, vendor/group-company sharing, security/access controls or cross-border data-specific compliance.
...
```text
Regulated role already committed; user asks only whether existing license covers Activity A
→ permission-entry-ongoing-compliance
```
...
### Perimeter/role precedes dependent BL7 propositions when unresolved

Where the regulated role/regime trigger is material and unresolved:

```text
regulatory-perimeter-role
→ committed / conditioned BL7 perimeter state
→ dependent permission / conduct / data proposition
```

A downstream BL7 unit must not silently invent the regulated role to continue.
...
- regulated activity/product/service/actor definitions;
- prohibited/conditional/permission/notification status;
- permit scope/lifecycle/ongoing conditions;
- consumer/customer mandatory rights/duties;
- advertising/claim/promotion/pricing restrictions;
- competition tests/thresholds/exemptions;
- privacy/data/monitoring roles, duties and conditions;
...
- BL7 specialist → BL7: candidate depth only; owner review required before shared-state promotion.
